...
from flask import Flask, Response, jsonify, render_template, request, session, redirect, url_for, make_response
...
known_faces_db = {
    ...
    "sumanth.jpg": {"name": "Sumanth ", "email": "saisumanth_oruganti@srmap.edu.in"},
    "Guna.jpg": {"name": "Guna vardhan", "email": "gunavardhan_byraju@srmap.edu.in"},
    "yaswanth.jpg": {"name": "Yaswanth ", "email": "yaswanth_seeram@srmap.edu.in"},
    "shankar.jpg": {"name": " shankar ", "email": "saisankar_yandamuri@srmap.edu.in"}
...
# Clear all Excel files at startup
for file_path in [attendance_file, present_file, absent_file]:
    ...
    wb = Workbook()
    ws = wb.active
    headers = ["SessionID", "Name", "Email", "Timestamp", "Status"] if file_path == attendance_file else ["Name", "Email", "Status"]
    ws.append(headers)
    wb.save(file_path)
...
def mark_attendance(name, email):
    if name != "Unknown" and name not in attendance_marked:
        ...
        try:
            ...
            ws = wb.active
            ws.append([current_session_id, name, email, timestamp, "Present"])
            wb.save(attendance_file)
            print(f"Attendance recorded: {name}, {email}, {timestamp}")
            attendance_marked.add(name)
        except Exception as e:
            print(f"Failed to write to attendance.xlsx: {e}")
...
def generate_frames():
    ...
    min_distance = 0.0  # Initialize min_distance to avoid undefined variable
    while True:
        if not is_running:
            frame = np.zeros((480, 640, 3), dtype=np.uint8)
            cv2.putText(frame, "Face Recognition Stopped", (50, 240), #for text in the black frame while stopped
        ...
        frame_count += 1
        small_frame = cv2.resize(frame, (0, 0), fx=0.5, fy=0.5)#resizing the frame to smaller one 
        ...
        if frame_count % process_every_n_frames == 0:
            ...
            for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
                ...
                distances.sort()
                min_distance, matched_name = distances[0] if distances else (1.0, "Unknown")
                threshold = 0.6
                margin = 0.1
...
def attendance_data():
    try:
        ...
        wb = load_workbook(attendance_file)
        ws = wb.active
        headers = ["SessionID", "Name", "Email", "Timestamp", "Status"]
        for row in ws.iter_rows(min_row=2, values_only=True):
            if row[0] == current_session_id:
                attendance_list.append(dict(zip(headers, row)))
        wb.close()
...
def verify_otp():
    ...
    if email in otp_storage and otp_storage[email] == entered_otp:
        ...
        response = make_response(jsonify({"message": "OTP verified successfully", "redirect": "/Home"}))
        response.headers['Cache-Control'] = 'no-store, no-cache, must-revalidate, max-age=0'
        response.headers['Pragma'] = 'no-cache'
        response.headers['Expires'] = '0'
        return response, 200
